...
int RBFTest_8(RecordBasedFileManager *rbfm) {
    ...
    prepareRecord(recordDescriptor.size(), nullsIndicator, 80, "AnteaterAnteaterAnteaterAnteaterAnteaterAnteaterAnteaterAnteaterAnteaterAnteater", 25, 177.8, 6201, record3, &recordSize);
    ...
    RBFM_ScanIterator rbfm_ScanIterator;
    	string conditionAttribute = "Salary";
...
//    	memcpy(value, &length ,4);
    	...
    	int str= 6201;
    	memcpy((char*)value,&str,4 );

    rc = rbfm->scan(fileHandle, recordDescriptor, conditionAttribute , GE_OP, value, vect, rbfm_ScanIterator);
